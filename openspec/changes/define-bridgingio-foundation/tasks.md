...
- [ ] 5.4 基于已确认的设计系统构建目标列表和会话详情界面，展示连接状态、环境指纹和能力摘要
- [ ] 5.5 构建可折叠命令时间线，展示命令摘要、stdout/stderr 入口、exit status 和 artifact 关联
- [ ] 5.6 构建 artifact 详情与二次过滤交互，使用户能够重新查看或扩大过滤范围
- [ ] 5.7 构建审批界面和状态反馈，覆盖等待审批、批准、拒绝和失败结果
- [ ] 5.8 为 macOS 控制台补充 UI Test，覆盖目标选择、会话查看、命令时间线、artifact 详情和审批流程
