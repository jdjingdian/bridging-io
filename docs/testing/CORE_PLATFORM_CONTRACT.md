...
- self-test: `bridgingio-core --self-test` validates key contract paths, including default model-plane bind probe on `127.0.0.1:19718`
...
| `CP-TRANSPORT` | control-plane attach semantics (unix) and host transport contract snapshot | `source/rust/bridgingio-mcp/tests/integration_workflows.rs` unix IPC tests + `bridgingio-core --self-test` step `[7/7]` |
| `CP-RUNTIME-PATHS` | host-aware runtime path resolution and endpoint contract | `source/rust/bridgingio-platform/src/lib.rs` unit tests + `bridgingio-core --self-test` step `[7/7]` |
...
| `CP-VAULT-LOGGER-DECODER` | native vault degraded semantics, logger categories, decoder normalization | `source/rust/bridgingio-platform/src/lib.rs` unit tests + `bridgingio-core --self-test` step `[7/7]` |
| `CP-SELF-TEST` | end-to-end CLI self-test smoke for contract-critical paths, including default model-plane bind diagnostics on `127.0.0.1:19718` | `cargo run -p bridgingio-mcp --bin bridgingio-core -- --self-test` |
