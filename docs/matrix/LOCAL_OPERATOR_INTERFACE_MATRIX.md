...
| CLI | `bridgingio-core menuconfig [--config <path>]` | local operator, future UI tooling | optional config path | interactive TUI session, saved config path | display-safe diagnostics only, no secret plaintext display; entering menuconfig must not trigger keychain unlock prompts until explicit unlock action (`u` or equivalent) | save returns equivalent restart hint when config changed |
