...
| Control Plane | `target credential source` (`Use Imported Vault SSH Key` / `Import Local SSH Key Into Vault` / `Manual Reference`) | trusted local UI/TUI host | target id + optional imported credential selection | target `credential_ref` is canonicalized only for `vault:` refs (`vault://.../ssh-private-key/<name>`); direct identity manual references are persisted as-is | sensitive+ssh flow is unlocked-only; locked sensitive target must not expose bound key identity; direct identity input must not be rewritten into vault refs | live action |
| CLI | `menuconfig -> Targets -> SSH target -> Test Connection` | local operator | current SSH draft fields (`host`/`port`/`username`/`credential_ref` etc.) + `timeout_ms` (default `2000`) | concise result popup (`Connection succeeded` / `Connection failed` / `Connection cancelled` / `Connection timed out`) | plain SSH entry is always visible in `Connection Profile`; sealed SSH entry is visible only in unlocked `Sensitive Overlay`; plain + vault-backed credential may return controlled preflight failure while vault is locked; broker endpoint is injected only after runtime bind/listen ready and failure is classified as broker-specific; direct identity path injects explicit `-i`/`IdentityFile` + `IdentitiesOnly=yes` + `IdentityAgent=none`, and failures must not be reported as broker unavailable; no implicit unlock; detailed diagnostics remain display-safe session logs only | live action |
